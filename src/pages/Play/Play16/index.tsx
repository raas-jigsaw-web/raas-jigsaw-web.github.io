...
import {solveMrvLocal} from '../../../services/jigsaw/mrvSolver';

/** Same UI as Play13/15; MRV solve runs in-browser (TypeScript port of API solver — no `/resolve` call). */
export default class PlayPage extends React.Component<any, any> {
  ...
  resolve = () => {
    ...
    const count = Math.max(Backboard.ResolveMinCount, dateResultCount + Backboard.LoadMore);
    const dj = dayjs(this.queryDate, 'MM/DD/YYYY');
    if (!dj.isValid()) {
      message.warning(formatMessage({ id: 'pages.play16.badDate' }));
      return;
    }
    const startedAt = performance.now();
    this.setState({ resolveLoading: true });
    try {
      const json = solveMrvLocal(dj.month(), dj.date() - 1, dj.day(), count);
      this.resultsCount[this.queryDate] = json.count || 0;
      const results: SquareProps[][][] = [];
      if (json && json.code === 0 && json.data && json.data.length > 0) {
        let boxesList: SquareProps[][] = [];
    ...
    return (<div>
      <div style={{padding: '2em 8em 0'}}>
        <Typography.Paragraph type="secondary" style={{marginBottom: 16}}>
          <FormattedMessage id="pages.play16.hint" />
